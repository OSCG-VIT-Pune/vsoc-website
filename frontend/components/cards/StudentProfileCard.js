...
import StudentSettingsModal from '../forms/StudentSettingsModal'
      ...
      <div className="border-4 border-cyan-600 bg-gradient-to-br from-gray-900 to-black p-6 rounded-none relative overflow-hidden group">
        ...
        <div className="absolute top-2 right-2 z-20">
          <button 
            ...
            className="text-xs font-pixel text-cyan-500 hover:text-cyan-300 border-2 border-cyan-800 hover:border-cyan-400 p-1 px-2 transition-all bg-black/50"
          >
            EDIT
          </button>
        </div>
        ...
        <div className="text-center mb-6 relative z-10">
          ...
          <h2 className="font-pixel text-xl text-white pixel-text uppercase">
            {user?.name || 'PLAYER ONE'}
          </h2>
          <div className="text-cyan-400 font-pixel text-xs">
            {user?.userType === 'student' ? 'CADET' : 'VISITOR'}
          </div>
        </div>
